...
exports.create = async (req, res) => {
  const { nom, age, genre, email, telephone, cin, antecedents, date_naissance } = req.body;
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO patients (nom, age, genre, email, telephone, cin, antecedents, date_naissance, date_creation)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW()) RETURNING *`,
      [nom, age, genre, email, telephone, cin, antecedents, date_naissance || null]
    );
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE patients SET nom=$1,age=$2,genre=$3,email=$4,telephone=$5,cin=$6,antecedents=$7,date_naissance=$8
       WHERE id=$9 RETURNING *`,
      [nom, age, genre, email, telephone, cin, antecedents, date_naissance || null, req.params.id]
    );
    res.json(rows[0]);
  } catch (err) {
    console.error(err.message);
