...
class _PatientDetailScreenState extends State<PatientDetailScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final examens = await db.getExamsByPatient(widget.patientId);
      final Map<int, Result?> resultats = {};
      for (var ex in examens) {
        try {
          resultats[ex.id!] = await db.getResultatByExam(ex.id!);
        } catch (_) {
      ...
      children: [
        _infoCard('Identité', [
          ...
          if (p.dateNaissance != null)
            _row(Icons.event_outlined, 'Date de naissance',
                '${p.dateNaissance!.day.toString().padLeft(2, '0')}/'
                '${p.dateNaissance!.month.toString().padLeft(2, '0')}/'
                '${p.dateNaissance!.year}'),
