...
class _ResultsScreenState extends State<ResultsScreen>
  ...
  Future<void> _load() async {
    try {
      final data = await _api.get('/results/exam/${widget.examId}');
      final list = data as List;
      if (list.isNotEmpty) {
        final r = Result.fromJson(list[0] as Map<String, dynamic>);
        setState(() {
          _result = r;
  ...
  void _copyReport() {
    ...
    final lines = <String>[
      ...
      if (findings.isNotEmpty) ...[
        'ANOMALIES DÉTECTÉES (p > 50%) :',
        findings,
        '',
      ],
      'Modèle  : EfficientNetB1 · NIH ChestX-ray14',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'EfficientNetB1 · NIH ChestX-ray14',
                      style: TextStyle(
                        fontSize: 10,
                        color: V4.inkMuted,
                        fontFamily: 'monospace',
    ...
    return ListView(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(_bannerIcon, color: sevColor, size: 20),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  _bannerText,
        ...
        _card(
          title: 'INFORMATIONS',
          children: [
            _row('Patient', patient.nom),
            _row('Âge / Genre',
                '${patient.age} ans · ${patient.genre}'),
            _row('Date', fmt(result.dateAnalyse)),
            _row('Diagnostic', result.diagnostic),
            _row('Confiance',
                '${result.confidence.toStringAsFixed(0)}%'),
            _row('Modèle', 'EfficientNetB1 · NIH ChestX-ray14'),
          ],
        ),
        ...
        if (alerts.isNotEmpty) ...[
          _card(
            title: 'ANOMALIES DÉTECTÉES (p > 50%)',
            children: alerts
                .map((e) => _row(
                    e.key,
