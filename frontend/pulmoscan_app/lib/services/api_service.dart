...
class ApiService {
  ...
  // Émis quand le serveur retourne 401 — MainNavigation redirige vers login
  static final _authErrorCtrl = StreamController<void>.broadcast();
  static Stream<void> get authErrors => _authErrorCtrl.stream;
  ...
  // adb reverse tcp:3000 tcp:3000 — phone localhost → PC port 3000
  ...
  dynamic _handle(http.Response res) {
    ...
    if (res.statusCode == 401) {
      logout(); // efface le token expiré
      _authErrorCtrl.add(null); // déclenche la redirection vers login
    }
    throw Exception(msg);
  }
